...
abstract public class TiledScene extends Scene {
    ...
    private final SparseIntArray CHARS;             // The list of chars used in the scene
    ...
    private final int tileSize;                     // Size of the tiles (both width and height)
    ...
    private void updateScales() {
        if (getGameEngine().getWidth() * getGameEngine().getHeight() != screenWidth * screenHeight) {
            screenWidth = getGameEngine().getWidth();
            screenHeight = getGameEngine().getHeight();
    ...
    public void draw(Canvas canvas) {
        ...
        // --- FIRST DRAW ROUND (translated & scaled)
        canvas.save();
        // Apply the scale
        ...
        // --- SECOND DRAW ROUND (game objects)
        super.draw(canvas);
